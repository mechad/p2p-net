	...
	while(1)
	{
        ...
        if (n>0)
        {
			...
			else if( msg.id == ACK )//链接请求被受理，只有发起者会收到该消息
			{
				...
				strcpy(message.user.srcinfo.ip, inet_ntoa(host_addr.sin_addr) );
				message.user.srcinfo.port = ntohs(host_addr.sin_port);
...
int main(int argc, char **argv)
{
	...
	while( 1 )
	{
		...
		else if( !strcmp( command, "sendto" ) ) 
		{
			...
			printf("destinfo.username=%s\n",message.user.destinfo.username);
			printf("send to ip=%s\n",userlist[No-1].ip);
			message.len = strlen(parameters[1]);
			memcpy( message.text, parameters[1], message.len );
